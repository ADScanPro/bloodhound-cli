#!/bin/bash
# Script para configurar testing usando SpecterOps bloodhound-cli oficial

set -e

echo "🎯 Setting up BloodHound CLI testing with SpecterOps tools"
...
if ! command -v bloodhound-cli &> /dev/null; then
    echo "❌ SpecterOps bloodhound-cli not found"
    echo "💡 Install it with: ./scripts/install-specterops-cli.sh"
    echo "💡 Or download from: https://github.com/SpecterOps/bloodhound-cli/releases/latest"
    exit 1
fi

echo "✅ Found SpecterOps bloodhound-cli"
...
if [ ! -f "$GOAD_ZIP" ]; then
    ...
    echo ""
    echo "🔄 Using synthetic test data instead..."
